...
Script for optimizing a Random Forest model using Particle Swarm Optimization (PSO) and generating visualizations.
Saves model-specific results in Results_PSO/RF.
Saves global results (figures, Excel, PDF) in Results_PSO.
Includes progress bars (tqdm) for PSO iterations.
Values on histograms are removed, and table values are formatted to 6 decimal places.
Cross-validation is used during hyperparameter optimization (configurable cv folds on 80% training data).
...
def get_initial_param_bounds(model_name):
    """Returns initial hyperparameter bounds for Random Forest."""
    logger.info(f"Retrieving bounds for {model_name}")
    bounds_dict = {
        'RF': {
            'numeric': {
                'n_estimators': (10, 300),
...
def generate_pdf_report(results_folder, optimized_params, df_metrics, method, image_size=(4*inch, 4*inch)):
    ...
    try:
        ...
        title = f"Machine Learning Model Analysis Report - {method}"
...
def visualize_model_results(model_folder, method, model_name, optimized_models):
    ...
    try:
        ...
        if not model:
            ...
            if not model_class:
                logger.error(f"Model {model_name} not defined.")
                return
...
def main(args):
    ...
    data_path = args.data_path or os.path.join(script_dir, 'Data_KI_Biaxial_Multi.xlsx')
    ...
    for name, config in tqdm(models.items(), desc="Processing models", unit="model"):
        ...
        visualize_model_results(results_folder, method, name, optimized_models)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run RF model optimization.")
    ...
    parser.add_argument('--selected_model', type=str, default='RF', help='Model to run.')
